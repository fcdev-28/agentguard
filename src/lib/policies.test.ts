...
describe("getPolicies", () => {
  it("ordena por severidad de estado (active > draft > archived)", () => {
    const input = [
      ...
      policy({ id: "active", status: "active", publishedAt: "2026-06-01T00:00:00.000Z" }),
    ];
    expect(getPolicies(input).map((p) => p.id)).toEqual(["active", "draft", "archived"]);
  });

  it("dentro de la misma severidad, ordena por publishedAt desc", () => {
    const input = [
      policy({ id: "old", status: "active", publishedAt: "2026-01-01T00:00:00.000Z" }),
      policy({ id: "new", status: "active", publishedAt: "2026-06-01T00:00:00.000Z" }),
    ];
  ...
  it("las políticas sin publicar quedan al final de su grupo de estado", () => {
    const input = [
      policy({ id: "unpublished", status: "active", publishedAt: null, updatedAt: "2026-07-01T00:00:00.000Z" }),
      policy({ id: "published", status: "active", publishedAt: "2026-01-01T00:00:00.000Z" }),
    ];
    expect(getPolicies(input).map((p) => p.id)).toEqual(["published", "unpublished"]);
  });
...
describe("getAffectedActions", () => {
  it("devuelve las acciones cuyas condiciones casan con la política, más recientes primero", () => {
    const pol = policy({ id: "p1", conditions: { actionType: "issue_refund" } });
    const input = [
      action({ id: "old", actionType: "issue_refund", createdAt: "2026-07-01T00:00:00.000Z" }),
      action({ id: "new", actionType: "issue_refund", createdAt: "2026-07-10T00:00:00.000Z" }),
      action({ id: "otro", actionType: "send_email", createdAt: "2026-07-11T00:00:00.000Z" }),
    ];
    expect(getAffectedActions(pol, input, emptyCtx).map((a) => a.id)).toEqual(["new", "old"]);
  });

  it("recorta al límite indicado", () => {
    const pol = policy({ id: "p1", conditions: { actionType: "issue_refund" } });
    const input = [
      action({ id: "a1", actionType: "issue_refund", createdAt: "2026-07-01T00:00:00.000Z" }),
      action({ id: "a2", actionType: "issue_refund", createdAt: "2026-07-05T00:00:00.000Z" }),
      action({ id: "a3", actionType: "issue_refund", createdAt: "2026-07-10T00:00:00.000Z" }),
    ];
    expect(getAffectedActions(pol, input, emptyCtx, 2).map((a) => a.id)).toEqual(["a3", "a2"]);
  });

  it("incluye una política en borrador (previsualización de edición sin publicar)", () => {
    const pol = policy({ id: "p1", status: "draft", conditions: { actionType: "issue_refund" } });
    const input = [action({ id: "a1", actionType: "issue_refund" })];
    expect(getAffectedActions(pol, input, emptyCtx).map((a) => a.id)).toEqual(["a1"]);
  });
