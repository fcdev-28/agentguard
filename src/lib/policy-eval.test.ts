...
describe("evaluatePolicy", () => {
  it("condición tool: casa cuando el tipo de la herramienta de la acción coincide", () => {
    const ctx: PolicyEvalContext = {
      ...emptyCtx,
      tools: [tool({ id: "t1", type: "billing" })],
    };
    const pol = policy({
      id: "p1",
      conditions: { tool: "billing" },
      effect: "block",
    });
  ...
  it("condición scope: casa cuando existe un permiso del agente con ese alcance sobre la herramienta", () => {
    const ctx: PolicyEvalContext = {
      ...emptyCtx,
      permissions: [
        permission({ agentId: "ag1", toolId: "t1", scope: "draft" }),
      ],
    };
    const pol = policy({
      id: "p1",
      conditions: { scope: "draft" },
      effect: "allow",
    });
  ...
  it("condición scope: no casa cuando no existe permiso con ese alcance", () => {
    const ctx: PolicyEvalContext = {
      ...emptyCtx,
      permissions: [
        permission({ agentId: "ag1", toolId: "t1", scope: "read" }),
      ],
    };
    const pol = policy({
      id: "p1",
      conditions: { scope: "write" },
      effect: "allow",
    });
    const act = action({ agentId: "ag1", toolId: "t1" });
    expect(evaluatePolicy(act, [pol], ctx).policyId).toBeNull();
  ...
  it("condición environment: no casa cuando el entorno difiere", () => {
    const ctx: PolicyEvalContext = {
      ...emptyCtx,
      agents: [agent({ id: "ag1", environment: "sandbox" })],
    };
    const pol = policy({
      id: "p1",
      conditions: { environment: "production" },
  ...
  it("precedencia: entre varias que casan, gana la de efecto más severo (block > escalate > require_approval > allow)", () => {
    ...
    const escalatePol = policy({
      id: "escalate",
      conditions: { environment: "production" },
      effect: "escalate",
    });
    const blockPol = policy({
      id: "block",
      conditions: { environment: "production" },
      effect: "block",
    });
    ...
    const result = evaluatePolicy(
      act,
      [allowPol, approvalPol, escalatePol, blockPol],
      ctx,
    );
    expect(result.policyId).toBe("block");
    expect(result.matched.map((p) => p.id).sort()).toEqual([
      "allow",
      "approval",
      "block",
      "escalate",
  ...
  it("desempate: las políticas sin publicar (publishedAt null) quedan al final", () => {
    ...
    const act = action({ agentId: "ag1" });
    expect(evaluatePolicy(act, [unpublished, published], ctx).policyId).toBe(
      "published",
    );
  });
  ...
  it("genera un reason legible a partir de la política ganadora", () => {
    const pol = policy({
      id: "p1",
      name: "Bloqueo de borrados",
      effect: "block",
    });
    const act = action({});
    expect(evaluatePolicy(act, [pol], emptyCtx).reason).toBe(
      "Bloqueo de borrados: Bloquear.",
    );
  });
});
