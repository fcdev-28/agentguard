...
describe("getAgents", () => {
  it("ordena por severidad de estado (error > active > paused > disabled)", () => {
    const input = [
      agent({ id: "paused", status: "paused", updatedAt: "2026-07-10T00:00:00.000Z" }),
      agent({ id: "disabled", status: "disabled", updatedAt: "2026-07-10T00:00:00.000Z" }),
      agent({ id: "error", status: "error", updatedAt: "2026-07-10T00:00:00.000Z" }),
      agent({ id: "active", status: "active", updatedAt: "2026-07-10T00:00:00.000Z" }),
    ];
  ...
  it("dentro de la misma severidad, ordena por updatedAt desc", () => {
    const input = [
      agent({ id: "old", status: "active", updatedAt: "2026-07-01T00:00:00.000Z" }),
      agent({ id: "new", status: "active", updatedAt: "2026-07-10T00:00:00.000Z" }),
    ];
...
describe("getAgentLastActivityAt", () => {
  it("devuelve el createdAt más reciente entre las acciones del agente", () => {
    const input = [
      action({ id: "a1", agentId: "ag1", createdAt: "2026-07-10T00:00:00.000Z" }),
      action({ id: "a2", agentId: "ag1", createdAt: "2026-07-12T00:00:00.000Z" }),
      action({ id: "a3", agentId: "otro", createdAt: "2026-07-13T00:00:00.000Z" }),
    ];
    expect(getAgentLastActivityAt(input, "ag1")).toBe("2026-07-12T00:00:00.000Z");
  });
...
describe("getAgentTools", () => {
  it("resuelve las herramientas conectadas a partir de los permisos del agente", () => {
    const tools = [tool({ id: "t1", name: "Correo" }), tool({ id: "t2", name: "CRM" })];
    const permissions = [
      permission({ id: "p1", agentId: "ag1", toolId: "t1", scope: "draft", status: "allowed" }),
      permission({ id: "p2", agentId: "ag1", toolId: "t2", scope: "write", status: "restricted" }),
  ...
  it("ignora permisos cuyo tool no existe", () => {
    ...
    const permissions = [permission({ id: "p1", agentId: "ag1", toolId: "t2" })];
...
describe("getAgentActions", () => {
  it("ordena las acciones del agente por createdAt desc", () => {
    const input = [
      action({ id: "old", agentId: "ag1", createdAt: "2026-07-10T00:00:00.000Z" }),
      action({ id: "new", agentId: "ag1", createdAt: "2026-07-12T00:00:00.000Z" }),
      action({ id: "otro", agentId: "otro", createdAt: "2026-07-13T00:00:00.000Z" }),
    ];
    expect(getAgentActions(input, "ag1").map((a) => a.id)).toEqual(["new", "old"]);
  });

  it("recorta al límite indicado", () => {
    const input = [
      action({ id: "a1", agentId: "ag1", createdAt: "2026-07-10T00:00:00.000Z" }),
      action({ id: "a2", agentId: "ag1", createdAt: "2026-07-11T00:00:00.000Z" }),
      action({ id: "a3", agentId: "ag1", createdAt: "2026-07-12T00:00:00.000Z" }),
    ];
    expect(getAgentActions(input, "ag1", 2).map((a) => a.id)).toEqual(["a3", "a2"]);
  });
